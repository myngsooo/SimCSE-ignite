...
def main():
    ...
    checkpoint = torch.load(args.model_name_or_path, map_location='cpu' if args.gpu_id < 0 else 'cuda:%d' % args.gpu_id)
    config = checkpoint['args']
    model_best = checkpoint['model_best']
    
    model = BertModel.from_pretrained(config.model_name_or_path)
